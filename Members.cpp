...
{
	for (short int i = 0; i < std::size(variable_type); i++) if (boost::algorithm::to_lower_copy(type) == variable_type[
		i]) return i;
	return -1;
}
...
{
	for (short int i = 0; i < std::size(keywords); i++) if (boost::algorithm::to_lower_copy(type) == keywords[i]) return
		i;
	return -1;
}
